...
def define_graph(glove_embeddings_arr):
    ...
    stacked_fw_lstm = tf.contrib.rnn.MultiRNNCell(
         [lstm_cell() for _ in range(number_of_layers)])

    stacked_bw_lstm = tf.contrib.rnn.MultiRNNCell(
         [lstm_cell() for _ in range(number_of_layers)])

    #
    # lstm_fw_Cell = tf.contrib.rnn.LSTMCell(lstmUnits)
    # lstm_bw_Cell = tf.contrib.rnn.LSTMCell(lstmUnits)

    data1 = tf.unstack(data1, maxSeqLength, 1)
    ...
    value, _, _ = tf.nn.static_bidirectional_rnn(stacked_fw_lstm,stacked_bw_lstm, data1, dtype=tf.float32)  ## define backprogation through time 40
    #value, _ = tf.nn.dynamic_rnn(lstmCell, data1, dtype=tf.float32) ## define backprogation through time 40
